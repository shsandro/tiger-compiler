...
static Temp_temp munchExp(T_exp e) {
    switch (e->kind) {
        case T_BINOP: {
            ...
            int float_op = (e1->kind == T_FCONST || e2->kind == T_FCONST);
            switch (e->u.BINOP.op) {
                case T_plus:
                    if (float_op)
                        op = String("add.s");
...
static Temp_tempList munchArgs(T_expList args) {
    ...
    for (; el; el = el->tail) {
        ...
        if (cnt < 4) {
            ...
            sprintf(buf, "sw `s0, %d(`s1)\n", cnt * 4);
            emit(AS_Oper(String(buf), NULL,
                         Temp_TempList(t, Temp_TempList(sp, NULL)), NULL));
        }
        cnt++;
    }
    return rlist;
